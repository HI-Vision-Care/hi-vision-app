...
import { Text, View, TouchableOpacity, Image, TextInput, ScrollView, StatusBar } from "react-native"
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1">
            ...
            {editable && !hasDropdown ? (
              ...
              <Text className="flex-1 text-gray-900 text-base font-medium">{value}</Text>
            )}
          </View>
          {hasEdit && (
            <TouchableOpacity onPress={() => handleEditField(field)} className="w-8 h-8 justify-center items-center">
              <Text className="text-gray-400 text-base">✏️</Text>
            </TouchableOpacity>
          )}
          {hasDropdown && (
            <TouchableOpacity onPress={() => handleDropdownPress(field)} className="w-8 h-8 justify-center items-center">
    ...
    <>
      ...
      <SafeAreaView className="flex-1 bg-gray-100">
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View className="px-5">
            {renderInputField("Full Name", personalData.fullName, "fullName", "👤")}
            {renderInputField("Email Address", personalData.email, "email", "✉️")}
            {renderInputField("Phone Number", personalData.phoneNumber, "phoneNumber", "📱")}
            {renderInputField("Date of Birth", personalData.dateOfBirth, "dateOfBirth", "📅", false, true, false)}
            {renderInputField("Address", personalData.address, "address", "📍")}
            {renderInputField("Country", personalData.country, "country", "🏳️", false, true, false)}
